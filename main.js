...
function resetPlotsUI(lock = true) {
  // clear any cached SVGs
  for (const k of Object.keys(svgCache)) delete svgCache[k];

  // revoke any live blob URL
  if (currentPlotURL) { try { URL.revokeObjectURL(currentPlotURL); } catch {} ; currentPlotURL = null; }

  // reset the UI
  softClearNode(el.plotDiv, '<div class="text-gray-500 p-3">Awaiting plot...</div>');
  if (el.plotTypeSel) el.plotTypeSel.value = "mic";

  // (optionally) disable plot types until fresh results exist
  ["plot-opt-mic","plot-opt-ratio","plot-opt-delta","plot-opt-dod-ratio","plot-opt-dod-delta"].forEach(id => {
    const o = document.getElementById(id);
    if (o) o.disabled = !!lock;
  });
}

// Pretty labels for on-screen UI (can use Unicode & HTML semantics)
const PRETTY_LABELS_UI = {
  // common
  MIC: "MIC",
  CI_Lower: "95% CI (lower)",
  CI_Upper: "95% CI (upper)",
  SE_LP: "Std. error (linear predictor)",

  // pairwise
  Ratio_MIC: "MIC ratio (fold change)",
  Delta_MIC: "ΔMIC",
  log2Ratio_MIC: "log\u2082(MIC ratio)",        // log₂(...)
  SE_log2Ratio: "Std. error [log\u2082 ratio]",

  // DoD (ΔΔ = difference-in-differences)
  DDlog2MIC: "ΔΔ log\u2082(MIC ratio)",
  DDMIC: "ΔΔMIC",
  SE_logDoD: "Std. error [ΔΔ log\u2082 ratio]",
  SE_DoD: "Std. error [ΔΔMIC]",

  // p-values & misc
  P_value: "p-value",
  "Pr(>|z|)": "p-value",
  Comparison: "Comparison",
  Observed_MIC: "Observed MIC",
  Censored_At: "Censored at",
};

// Export labels (CSV/XLSX): ASCII-friendly, no subscripts
const PRETTY_LABELS_EXPORT = {
  MIC: "MIC",
  CI_Lower: "95% CI (lower)",
  CI_Upper: "95% CI (upper)",
  SE_LP: "Std. error (linear predictor)",
  ...
  Ratio_MIC: "MIC ratio (fold change)",
  Delta_MIC: "Delta MIC",
  log2Ratio_MIC: "log2(MIC ratio)",
  SE_log2Ratio: "Std. error [log2 ratio]",

  DDlog2MIC: "Delta-Delta log2(MIC ratio)",
  DDMIC: "Delta-Delta MIC",
  SE_logDoD: "Std. error [Delta-Delta log2 ratio]",
  SE_DoD: "Std. error [Delta-Delta MIC]",
...
// keep your regex but it already handles "p-value"
const P_COL_REGEX = /^(p(?:-?value)?|P_value|Pr\(>\|z\|\))$/i;
function isPColumn(h) { return P_COL_REGEX.test(h); }

// turn exact zero p's into 1e-308 (so Excel/plots don't see 0)
function forceTinyP(rows = []) {
  return rows.map(r => {
    const o = { ...r };
    for (const k of Object.keys(o)) {
      if (isPColumn(k) && o[k] === 0) o[k] = 1e-308;
      // also catch already-prettified header text "p-value" if present
      if (String(k).toLowerCase() === "p-value" && o[k] === 0) o[k] = 1e-308;
    }
    return o;
  });
}

function prettifyRows(rows = [], map) {
  return rows.map(r => {
    const o = {};
...
function prettifyRowsUI(rows)     { return prettifyRows(rows, PRETTY_LABELS_UI); }
function prettifyRowsExport(rows) { return prettifyRows(rows, PRETTY_LABELS_EXPORT); }

// one-liners you’ll use below
function prepareForExport(rows) { return prettifyRowsExport(forceTinyP(rows)); }
...
async function resetApp() {
  ...
  // NEW: clear file input & reset example button text
  if (el.csvInput) el.csvInput.value = "";
  if (el.exampleBtn) {
    el.exampleBtn.disabled = false;
    el.exampleBtn.textContent = "Load Example Dataset";
  }
  ...
  const cmpAll = document.getElementById("cmp-all");
if (cmpAll) cmpAll.checked = true;
  ...
  // Reset replicate filter UI
  if (el.replicateNote) el.replicateNote.classList.add("hidden");

  // R side: drop globals created during run (not just our helpers)
  try {
    await window.webR.evalRVoid(`
      for (nm in c(
        "df","model","mic_analysis","s","mic_df","ratio_df","delta_df","dod_ratio_df","dod_delta_df",
        "formula_str","session_info","final_list",
        ".mic_env",".build_plot",".render_plot_svg"
      )) {
...
function renderSVG(svgString) {
  ...
  // NEW: revoke the previous URL to avoid stale images & leaks
  if (currentPlotURL) {
    try { URL.revokeObjectURL(currentPlotURL); } catch (_) {}
  }
  currentPlotURL = url;

  el.plotDiv.innerHTML = ""; // clear container
  const img = new Image();
  // Optional: revoke once loaded too (double-safety; harmless if already revoked)
...
function prettifyModelSummaryUI(ms) {
  return {
    coefficients: prettifyRowsUI(forceTinyP(ms?.coefficients || [])),
    thresholds:   prettifyRowsUI(ms?.thresholds || [])
  };
}
...
function isWithinMode() {
  // prefer new radio; fall back to the legacy checkbox if present
  const r = document.querySelector('input[name="comparison-mode"]:checked');
  if (r) return r.value === "within";
  return !!el.filterPairs?.checked; // backward compatibility
}
...
function setupData(data, isExample = false) {
  ...
  // Show replicate notice if 'ReplicateNumber' exists; default ON
  const hasReplicate = cols.includes("ReplicateNumber");
  
  // Show a friendly notice if present
  if (el.replicateNote) el.replicateNote.classList.toggle("hidden", !hasReplicate);
...
function setupDownloadListeners() {
  ...
  $("download-summary-coeffs").onclick = () =>
    downloadCSV(prepareForExport(last.model_summary.coefficients), "model_coefficients.csv");
  $("download-summary-thresholds").onclick = () =>
    downloadCSV(prettifyRowsExport(last.model_summary.thresholds), "model_thresholds.csv");

  $("download-mic").onclick = () =>
    downloadCSV(prettifyRowsExport(last.mics), "mic_estimates.csv");
  $("download-delta-mic").onclick = () =>
    downloadCSV(prepareForExport(last.delta_mics), "delta_mic.csv");
  $("download-ratio-mic").onclick = () =>
    downloadCSV(prepareForExport(last.ratio_mics), "ratio_mic.csv");
}
...
function zeroToMinP(rows) {
  if (!Array.isArray(rows)) return rows;
  return rows.map(row => {
    const out = { ...row };
    for (const k of Object.keys(out)) {
      if (isPColumn(k)) {
        const x = Number(out[k]);
        if (Number.isFinite(x) && x === 0) out[k] = P_MIN; // keep numeric
      }
    }
    return out;
  });
...
function buildWorkbook() {
  ...
  XLSX.utils.book_append_sheet(wb, arrayToSheet(prettifyRowsExport(last.mics)),        "MIC");
  XLSX.utils.book_append_sheet(wb, arrayToSheet(prettifyRowsExport(last.observed_mic)), "ObservedMIC");
  XLSX.utils.book_append_sheet(wb, arrayToSheet(prepareForExport(last.delta_mics)),    "Delta MIC");
  XLSX.utils.book_append_sheet(wb, arrayToSheet(prepareForExport(last.ratio_mics)),    "MIC ratio");
  XLSX.utils.book_append_sheet(wb, arrayToSheet(prepareForExport(last.dod_ratio)),     "DeltaDelta (ratio)");
  XLSX.utils.book_append_sheet(wb, arrayToSheet(prepareForExport(last.dod_delta)),     "DeltaDelta (Delta MIC)");
  XLSX.utils.book_append_sheet(wb, arrayToSheet(prepareForExport(last.model_summary?.coefficients || [])), "Coefficients");
  XLSX.utils.book_append_sheet(wb, arrayToSheet(prettifyRowsExport(last.model_summary?.thresholds || [])), "Thresholds");
...
# Keep 1 random technical replicate per unique cell if ReplicateNumber exists (deterministic)
if ("ReplicateNumber" %in% names(df)) {
  set.seed(1234)
  score_col <- "${score}"
  conc_col  <- "${conc}"
  gvars <- c(${fac1 ? `"${fac1}"` : "NULL"},
             ${fac2 ? `"${fac2}"` : "NULL"})
  gvars <- gvars[!is.na(gvars)]
  cell_cols <- intersect(c(gvars, conc_col), names(df))
  if (length(cell_cols) == 0) cell_cols <- setdiff(names(df), c("ReplicateNumber", score_col))

  df <- df |>
    dplyr::filter(!is.na(.data$ReplicateNumber)) |>
...
coef_df <- tibble::rownames_to_column(as.data.frame(summary(fit)$coefficients), "Term")
...
# --- enforce x = factor1, color = factor2 order in the mic object ---
desired_vars <- c(
  ${el.fac1Sel.value ? JSON.stringify(el.fac1Sel.value) : "NULL"},
  ${el.fac2Sel.value ? JSON.stringify(el.fac2Sel.value) : "NULL"}
)
desired_vars <- desired_vars[!is.na(desired_vars)]

if (is.list(mic) && "newdata" %in% names(mic) && is.data.frame(mic$newdata)) {
  present <- intersect(desired_vars, names(mic$newdata))
  if (length(present) > 0) {
    others <- setdiff(names(mic$newdata), present)
    mic$newdata <- mic$newdata[, c(present, others), drop = FALSE]
  }
  if ("group_vars" %in% names(mic)) mic$group_vars <- present
}
...
# Observed MIC by reading the autoplot points layer (one row per series)
get_observed_from_autoplot <- function(ma) {
  p <- ggplot2::autoplot(ma, type = "mic")
  idx <- NA_integer_
  for (i in seq_along(p$layers)) {
    dat <- p$layers[[i]]$data
    if (is.null(dat)) next
    if (all(c("y","cens") %in% names(dat))) { idx <- i; break }
  }
  if (is.na(idx)) return(tibble::tibble())

  pts <- p$layers[[idx]]$data
  group_vars <- c(
    ${el.fac1Sel.value ? JSON.stringify(el.fac1Sel.value) : "NULL"},
    ${el.fac2Sel.value ? JSON.stringify(el.fac2Sel.value) : "NULL"}
  )
  group_vars <- group_vars[!is.na(group_vars) & group_vars %in% names(ma$newdata)]


  if ("x_val" %in% names(pts) && length(group_vars) >= 1)
    names(pts)[names(pts)=="x_val"] <- group_vars[1]
  if ("color_val" %in% names(pts)) {
    if (length(group_vars) >= 2) {
      names(pts)[names(pts)=="color_val"] <- group_vars[2]
    } else {
      pts$color_val <- NULL
    }
  }

  pts$Observed_MIC <- ifelse(!is.na(pts$y) & !pts$cens, as.numeric(pts$y), NA_real_)
  pts$Censored_At  <- ifelse(!is.na(pts$y) &  pts$cens, as.numeric(pts$y), NA_real_)

  keep <- unique(c(group_vars[1],
                   if (length(group_vars) >= 2) group_vars[2],
                   "Observed_MIC","Censored_At"))
  tibble::as_tibble(pts[keep])
}

observed_out <- tryCatch(get_observed_from_autoplot(mic), error = function(e) tibble::tibble())
...
## Ensure no 0 p-values

\`\`\`r
excel_min <- .Machine$double.xmin  # ~2.225074e-308

clamp_p_cols <- function(df) {
  if (is.null(df) || !is.data.frame(df) || !nrow(df)) return(df)
  p_cols <- grep("^(p(_?value)?|P_value|P|Pr\\(>\\|z\\|\\))$", names(df),
                 perl = TRUE, value = TRUE)
  if (!length(p_cols)) return(df)
  for (nm in p_cols) {
    # clamp positive p's below Excel's minimum
    x <- suppressWarnings(as.numeric(df[[nm]]))
    too_small <- !is.na(x) & x > 0 & x < excel_min
    df[[nm]] <- ifelse(too_small, excel_min, x)

    # optional helpers for readability/plotting
    disp <- ifelse(too_small, "<1e-308", sprintf("%.3e", x))
    nlog <- ifelse(is.na(x) | x <= 0, NA_real_, -log10(pmax(x, excel_min)))
    df[[paste0(nm, "_display")]]  <- disp     # character
    df[[paste0(nm, "_neglog10")]] <- nlog     # numeric
  }
  df
}

coef_df      <- clamp_p_cols(coef_df)
thr_df       <- clamp_p_cols(thr_df)
ratio_out    <- clamp_p_cols(ratio_out)
delta_out    <- clamp_p_cols(delta_out)
dodR_out     <- clamp_p_cols(dodR_out)
dodD_out     <- clamp_p_cols(dodD_out)
\`\`\`
...
if (requireNamespace("writexl", quietly = TRUE)) {
  ...
  readr::write_csv(observed_out,  "results/ObservedMIC.csv")
...
- results/tables.xlsx         : MIC, DeltaMIC, RatioMIC, DoD_Ratio, DoD_Delta, ObservedMIC, plus Coefficients and Thresholds
...
- If a ReplicateNumber column is present, the analysis keeps one random technical replicate per unique cell (deterministic seed = 1234).
`;
}
...
async function downloadBundle() {
  ...
  try {
    ...
    const { tf: TF_CODE_RMD, inv: INV_CODE_RMD } =
      pickTransform(params.rhs, params.conc);
    
    const rmd = buildRmdText(
      params,
      fileMap,
      TF_CODE_RMD,
      INV_CODE_RMD,
...
el.runBtn.addEventListener("click", async () => {
  ...
  const cmpArg = isWithinMode() ? ", compare_pairs = 'share_any'" : "";
  ...
  const rCode = `
    tryCatch({
      ...
      # Keep one random technical replicate per (group vars x concentration) cell
      if ("ReplicateNumber" %in% names(df)) {
        set.seed(1234)
      
        # be explicit about the columns that define a "cell" (NO score here)
        score_col <- ${JSON.stringify(el.scoreSel.value)}
        conc_col  <- ${JSON.stringify(el.concSel.value)}
        gvars <- c(
          ${el.fac1Sel.value ? JSON.stringify(el.fac1Sel.value) : "NULL"},
          ${el.fac2Sel.value ? JSON.stringify(el.fac2Sel.value) : "NULL"}
        )
        gvars <- gvars[!is.na(gvars) & gvars %in% names(df)]
        cell_cols <- intersect(c(gvars, conc_col), names(df))
      
        # coerce ReplicateNumber (avoid "1" vs 1 mismatches)
        df$ReplicateNumber <- suppressWarnings(as.integer(as.character(df$ReplicateNumber)))
      
        # sample 1 row per cell across replicate numbers
        df <- df |>
          dplyr::filter(!is.na(.data$ReplicateNumber)) |>
          dplyr::group_by(dplyr::across(dplyr::all_of(cell_cols))) |>
          dplyr::slice_sample(n = 1) |>
      ...
      # types after replicate filtering
      ...
      # keep a CSV of the *exact* data used in modeling (post-filter)
      ...
      if (is.list(mic_analysis) && "newdata" %in% names(mic_analysis) && is.data.frame(mic_analysis$newdata)) {
        present <- intersect(desired_vars, names(mic_analysis$newdata))
        if (length(present) > 0) {
          others <- setdiff(names(mic_analysis$newdata), present)
          mic_analysis$newdata <- mic_analysis$newdata[, c(present, others), drop = FALSE]
        }
        if ("group_vars" %in% names(mic_analysis)) mic_analysis$group_vars <- present
      }
      ...
      normalize_comparison <- function(df) {
        if (is.null(df) || !is.data.frame(df) || !nrow(df)) return(df)
        # If 'Comparison' missing, promote 'label' or 'contrast' if present; else synthesize
        if (!"Comparison" %in% names(df)) {
          if ("label" %in% names(df)) {
            df <- dplyr::rename(df, Comparison = label)
          } else if ("contrast" %in% names(df)) {
            df <- dplyr::rename(df, Comparison = contrast)
          } else {
            df$Comparison <- "Difference-of-Differences"
          }
        } else if ("label" %in% names(df)) {
          # Fill any missing Comparison from label (if both exist)
          df$Comparison <- dplyr::coalesce(df$Comparison, df$label)
        }
        df %>%
          dplyr::select(-dplyr::any_of(c("label","contrast","var1","var2",
                                         "var1_lvlA","var1_lvlB","var2_lvlC","var2_lvlD")))
      }
      
      dod_ratio_df <- normalize_comparison(dod_ratio_df)
      ...
      dod_delta_df <- normalize_comparison(dod_delta_df)
      
      # ---- Observed MIC by reading the autoplot layer (one row per series) ----
      get_observed_from_autoplot <- function(ma) {
        p <- ggplot2::autoplot(ma, type = "mic")
      
        # find the points layer (has columns y + cens)
        idx <- NA_integer_
        for (i in seq_along(p$layers)) {
          dat <- p$layers[[i]]$data
          if (is.null(dat)) next
        ...
        # rename aesthetics back to real factor names
        if ("x_val" %in% names(pts) && length(group_vars) >= 1) {
          names(pts)[names(pts)=="x_val"] <- group_vars[1]
        }
        if ("color_val" %in% names(pts)) {
        ...
        # split 'y' into Observed_MIC vs Censored_At
        pts$Observed_MIC <- ifelse(!is.na(pts$y) & !pts$cens, as.numeric(pts$y), NA_real_)
        pts$Censored_At  <- ifelse(!is.na(pts$y) &  pts$cens, as.numeric(pts$y), NA_real_)
        ...
        keep <- unique(c(group_vars[1],
                         if (length(group_vars) >= 2) group_vars[2],
                         "Observed_MIC","Censored_At"))
        dplyr::as_tibble(pts[keep])
      }
      
      observed_df <- tryCatch(get_observed_from_autoplot(mic_analysis),
  ...
  try {
    ...
    renderModelSummary(el.modelSummary, prettifyModelSummaryUI(last.model_summary));
    renderTable(el.micTable, prettifyRowsUI(last.mics), "No MIC estimates were generated.");


    el.pairwiseBlock.classList.toggle("hidden", !last.ratio_mics?.length && !last.delta_mics?.length);
    if (last.delta_mics?.length) {
      renderTable(el.deltaTable, prettifyRowsUI(last.delta_mics), "No additive comparisons generated.");
    }
    if (last.ratio_mics?.length) {
      renderTable(el.ratioTable, prettifyRowsUI(last.ratio_mics), "No ratio comparisons generated.");
    }
    ...
    if (last.dod_ratio?.length) {
      renderTable(
        el.dodRatioTable,
        prettifyRowsUI(projectColumns(last.dod_ratio, ["Comparison","DDlog2MIC","SE_logDoD","CI_Lower","CI_Upper","P_value"])),
    ...
    if (last.dod_delta?.length) {
      renderTable(
        el.dodDeltaTable,
        prettifyRowsUI(projectColumns(last.dod_delta, ["Comparison","DDMIC","SE_DoD","CI_Lower","CI_Upper","P_value"])),
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // pretty plot option labels
  const PLOT_LABELS_UI = {
    mic: "MIC",
    ratio: "MIC ratio (log\u2082)",
    delta: "ΔMIC",
    dod_ratio: "ΔΔ (ratio)",
  ...
  const ids = {
    mic: "plot-opt-mic",
    ratio: "plot-opt-ratio",
    delta: "plot-opt-delta",
    dod_ratio: "plot-opt-dod-ratio",
    dod_delta: "plot-opt-dod-delta",
  };
  for (const [key, id] of Object.entries(ids)) {
    const opt = document.getElementById(id);
    if (opt) opt.textContent = PLOT_LABELS_UI[key];
  }
